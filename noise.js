...
const settings = {
  ...
  export: true,
  filename: 'creative-coding',
};

const sketch = ({ width, height }) => {
  const count = 40;
  const margin = width * 0.15;
  const maxColors = random.rangeFloor(611, 2220);
  const fontFamily = '"Andale Mono"';
  const palette = random.shuffle(random.pick(palettes)).slice(-6, maxColors);
  const background = 'hsl(0, 0%, 94%)';
  const characters = '=/-.vida|\\==|/'.split('');
